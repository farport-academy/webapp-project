...
export const globalRoutes: Routes = [
    {
      ...
      {
        path: 'about',
        loadComponent: () => import('./pages/components/about/about.component').then(m => m.AboutComponent)
      },
      {
        path: 'recipes',
        children: [
          {
            path: '',
            loadComponent: () => import('./pages/components/recipes/recipes.component').then(m => m.RecipesComponent)
          },
          {
            path: ':id',
            loadComponent: () => import('./pages/components/recipe-detail/recipe-detail.component').then(m => m.RecipeDetailComponent)
          },
        ],
      },
    {
        path: 'nested',
        loadChildren: () => import('./pages/components/nested/nested-routes.module').then(
          m => m.NestedModule
        )
    }
]
